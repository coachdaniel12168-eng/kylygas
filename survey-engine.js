...
async function runSurvey(){
  ...
  if(email&&email.includes('@')&&email.includes('.')){
    ...
    try{
      await Promise.all([
  fetch('/n8n-relay?w=kylygaseo-submit',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({url:validUrls[0],email:email,name:document.getElementById('industry').value.trim()||'Lead',country:'SG',status:'Free Audit Completed',score:clientData?clientData.overall:null}),signal:AbortSignal.timeout(5000)}),
  fetch('/n8n-relay?w=rankgas-welcome-v2',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email:email,name:document.getElementById('industry').value.trim()||'there'}),signal:AbortSignal.timeout(5000)})
